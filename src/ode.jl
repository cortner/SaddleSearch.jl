using Parameters, Printf, LinearAlgebra
import Dates

import SaddleSearch: IterationLog
...
SADDLESEARCH: ------|-----|-----------------\n", rtol,threshold)
   ...
   r = norm(Fn ./ max.(abs.(X), threshold), Inf) + eps(Float64)
   if h == nothing
      h = 0.5 * rtol^(1/2) / r
      h = max(h, hmin)
   end
   ...
   for nit = 1:maxnit
      ...
      # error estimation
      e = 0.5 * h * (Fnew - Fn)
      # err = norm(e ./ max.(maximum([abs.(X) abs.(Xnew)],2), threshold), Inf) + eps(Float64)
      err = norm(e, Inf) + eps(Float64)
