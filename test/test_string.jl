...
path = StaticString(0.0009, tol, maxnit, preconI, serial(), false, 1)
PATHx, PATHlog = run!(path, E, dE, Path(x))
...
path = ODEString(reltol=0.1, tol = tol, maxnit = maxnit,
                        ...
                        verbose = 1)
PATHx, PATHlog = SaddleSearch.run!(path, E, dE, Path(x))
@test PATHlog[:maxres][end] <= path.tol
...
path = StaticString(1./c, tol, maxnit, preconI, serial(), false, 1)
PATHx, PATHlog = run!(path, E, dE, Path(x))
@test PATHlog[:maxres][end] <= path.tol
...
path = StaticNEB(0.07, 0.1, 3, tol, maxnit, preconI, serial(), false, 1)
PATHx, PATHlog = run!(path, E, dE, Path(x))
@test PATHlog[:maxres][end] <= path.tol
...
path = StaticString(0.25, tolP, maxnit, preconP, serial(), false, 1)
PATHx, PATHlog = run!(path, E, dE, Path(x))
@test PATHlog[:maxres][end] <= path.tol
...
path = StaticNEB(0.6, 0.00001, 1, tolP, maxnit, preconP, serial(), false, 1)
PATHx, PATHlog = run!(path, E, dE, Path(x))
@test PATHlog[:maxres][end] <= path.tol
...
path = StaticNEB(0.001, 0.01, 3, tol, maxnit, preconI, serial(), false, 1)
PATHx, PATHlog = run!(path, E, dE, Path(x))
@test PATHlog[:maxres][end] <= path.tol
...
path = StaticNEB(1.0, 0.001, 3, tolP, maxnit, preconP, serial(), false, 1)
PATHx, PATHlog = run!(path, E, dE, Path(x))
@test PATHlog[:maxres][end] <= path.tol
